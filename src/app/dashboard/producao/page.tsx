      ...
      <Tabs defaultValue="producao" className="w-full">
        ...
        <TabsContent value="producao" className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-black uppercase tracking-tight">Fila de Produção</h2>
              <p className="text-[10px] font-bold uppercase text-muted-foreground">Baixa automática de insumos ao concluir</p>
            </div>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              <MapPin className="w-3.5 h-3.5" /> Agrupar Cidade
            </Button>
          </div>
          ...
          {Object.keys(groupedProduction).map((city, idx) => {
            ...
            return (
              <div key={city} className={idx > 0 ? 'border-t-4 border-primary/10 pt-4' : ''}>
                {groupByCity && (
                  <div className="bg-primary/5 px-4 py-2 border rounded-t-lg flex items-center justify-between">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2">
                      <MapPin className="w-3.5 h-3.5" /> {city}
                    </h3>
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-[9px] bg-white">{cityOrders.length} OP's</Badge>
                      <Badge variant="outline" className="text-[9px] bg-white">{totalUnits} UN</Badge>
                      <Badge variant="outline" className="text-[9px] bg-white">{totalKgCity.toFixed(2)} KG</Badge>
                    </div>
                  </div>
                )}
                <Card className={`border-none shadow-md overflow-hidden ${groupByCity ? 'rounded-tl-none rounded-tr-none' : ''}`}>
                  <CardContent className="p-0">
                    <Table>
                      <TableHeader className="bg-muted/50">
                        <TableRow>
                          <TableHead className="text-[9px] font-black uppercase">OP / Cliente</TableHead>
                          <TableHead className="text-[9px] font-black uppercase">Itens do Pedido</TableHead>
                          <TableHead className="text-[9px] font-black uppercase w-36">Progresso</TableHead>
                          <TableHead className="text-[9px] font-black uppercase text-right w-56">Ações</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {cityOrders.length === 0 && (
                          <TableRow>
                            <TableCell colSpan={4} className="text-center py-16 text-muted-foreground italic text-xs uppercase opacity-40">
                            ...
                            <TableRow key={order.id} className="hover:bg-muted/20 align-top">
                              <TableCell className="py-4">
                                <div className="flex flex-col gap-0.5">
                                  <span className="font-mono font-black text-primary text-xs">OP-{order.id.split('-')[1]}</span>
                                  ...
                                  <div className="flex gap-2 mt-1">
                                    <Badge variant="outline" className="text-[8px] font-black">{totalOrderUnits} UN</Badge>
                                    <Badge variant="outline" className="text-[8px] font-black">{totalOrderKg.toFixed(2)} KG</Badge>
                                  </div>
                                </div>
                              </TableCell>
                              <TableCell className="py-4">
                                <div className="flex flex-col gap-2">
                                  {order.items.map(item => {
                                    const prod = products.find(p => p.id === item.productId);
                                    return (
                                      <div key={item.productId} className="text-[10px] bg-white border rounded-lg px-3 py-2 max-w-xs">
                                        <p className="font-black text-primary uppercase text-[10px]">{prod?.name}</p>
                                        <div className="flex gap-4 mt-1 text-[9px] text-muted-foreground font-bold">
                              ...
                              <TableCell className="py-4">
                                <div className="space-y-1.5">
                                  <Progress value={getProgressValue(stage)} className="h-1.5" />
                                  <Badge variant="secondary" className={`${getStageColor(stage)} text-[8px] font-black uppercase h-4`}>
                                    {stage}
                                  </Badge>
                                </div>
                              </TableCell>
                                ...
                                <div className="flex justify-end gap-1.5">
                                  {stage === 'FILA' && (
                                    <Button size="sm" variant="outline"
                                      className="h-8 gap-1.5 font-black text-[9px] uppercase border-2"
                                      onClick={() => setSelectedOrderDetails(order)}>
                                      <Info className="w-3.5 h-3.5" /> Visualizar Pedido
                                    </Button>
                                  )}
                                  {stage === 'PROCESSO' && (
                                    <div className="flex gap-1.5">
                                      ...
                                      <Button size="sm" variant="outline"
                                        className="h-8 gap-1.5 font-black text-[9px] uppercase border-2 text-blue-600 border-blue-200 hover:bg-blue-50"
                                        onClick={() => {
                                          updateProductionStage(order.id, 'QUALIDADE');
                                          toast({ title: "Pedido Conferido", description: "Aguardando aprovação final." });
                                        }}>
                                        <ArrowRight className="w-3.5 h-3.5" /> Confirmar
                                      </Button>
                                    </div>
                                  )}
                                  {stage === 'QUALIDADE' && (
                                    <Button size="sm"
                                      ...
                                      <ShieldCheck className="w-3.5 h-3.5" /> Aprovar Produção
                                    </Button>
                                  )}
                                </div>
                              </TableCell>
        ...
        <TabsContent value="historico" className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-black uppercase tracking-tight">Histórico de Produção</h2>
              <p className="text-[10px] font-bold uppercase text-muted-foreground">Todos os pedidos processados</p>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-[9px] font-bold text-muted-foreground uppercase">
                {historicoFiltered.length} pedidos
              </span>
              <Button variant="outline" size="sm" className="gap-2 font-bold uppercase text-[10px]" onClick={handleExportHistorico}>
          ...
          <div className="bg-white border rounded-xl p-4 space-y-3">
            ...
            <div className="flex gap-4">
              <div className="flex items-center gap-2">
                ...
                <Input type="date" className="h-8 text-xs w-36" value={historicoDe} onChange={e => setHistoricoDe(e.target.value)} />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-[9px] font-black uppercase text-muted-foreground whitespace-nowrap">Até:</label>
                <Input type="date" className="h-8 text-xs w-36" value={historicoAte} onChange={e => setHistoricoAte(e.target.value)} />
              </div>
            </div>
          </div>
              ...
              <Table>
                <TableHeader className="bg-muted/50">
                  <TableRow>
                    ...
                    <TableHead className="text-[9px] font-black uppercase text-center">Aprovado</TableHead>
                ...
                <TableBody>
                  {historicoFiltered.map((order) => {
                    ...
                    return (
                      <TableRow key={order.id} className="hover:bg-muted/20 h-12">
                        ...
                        <TableCell className="text-center text-[10px] font-black">{totalUnitsOrder} un</TableCell>
          ...
          {selectedOrderDetails && (
            <div className="flex flex-col max-h-[90vh]">
              <div className="bg-primary px-8 py-5 flex items-center justify-between shrink-0">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-white/50 mb-0.5">{selectedOrderDetails.id}</p>
                  <h2 className="text-xl font-black uppercase text-white tracking-tight">{selectedOrderDetails.customerName}</h2>
                  <p className="text-[10px] font-bold text-white/60 uppercase mt-0.5">{selectedOrderDetails.city}</p>
                </div>
                <div className="flex items-center gap-2">
                  {selectedOrderDetails.productionStage === 'FILA' && (
                    <Button size="sm"
                      className="h-8 gap-1.5 font-black text-[9px] uppercase bg-orange-600 hover:bg-orange-700"
                      onClick={() => {
                      ...
                      <ArrowRight className="w-3.5 h-3.5" /> Aprovar Pedido
                    </Button>
                  )}
                  {selectedOrderDetails.productionStage === 'PROCESSO' && (
                    <Button size="sm"
                      className="h-8 gap-1.5 font-black text-[9px] uppercase bg-blue-600 hover:bg-blue-700"
                      onClick={() => {
                ...
                <div className="grid grid-cols-2 divide-x">
                  <div className="p-8 space-y-7">
                    <div className="space-y-3">
                      <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground pb-2 border-b">Cliente</p>
                      {[
                    ...
                    <div className="space-y-3">
                      <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground pb-2 border-b">Comercial</p>
                      {[
                  ...
                  <div className="p-8 flex flex-col gap-6">
                    <div className="space-y-3">
                      <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground pb-2 border-b">Itens do Pedido</p>
                          ...
                          return (
                            <div key={item.productId} className={`flex items-center justify-between py-3 px-4 rounded-lg ${idx % 2 === 0 ? 'bg-muted/40' : ''}`}>
                              ...
                              <div className="text-right shrink-0 ml-4">
                                <p className="text-[10px] font-black">{item.quantity} un × R$ {item.price.toLocaleString()}</p>
                    ...
                    <div className="mt-auto pt-4 border-t space-y-2">
                      ...
                      <div className="flex justify-between text-[10px] font-bold text-muted-foreground">
                        <span>Estágio</span>
                        <Badge className={`${getStageColor(selectedOrderDetails.productionStage || 'FILA')} text-[8px] font-black uppercase px-2 h-5`}>
                          {selectedOrderDetails.productionStage || 'FILA'}
                        </Badge>
                      </div>
                      <div className="flex justify-between items-baseline pt-2 border-t">
                        <span className="text-xs font-black uppercase text-primary">Total</span>
                        <span className="text-3xl font-black text-primary">R$ {selectedOrderDetails.totalValue.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
